...
const Solar = () => {
    ...
    const schema = useMemo(() => {
        return yup.object().shape({
            panelAmount: yup
                .number()
                .typeError('Panel amount must be a valid number')
                .required('Panel amount is required')
                .positive('Panel amount must be positive'),

            panelSize: yup
                .number()
                .typeError('Panel size must be a valid number')
                .required('Panel size is required')
                .positive('Panel size must be positive'),

            totalKW: yup
                .number()
                .typeError('Total KW must be a valid number')
                .required('Total KW is required')
                .positive('Total KW must be positive'),

            // batteryCapacity: yup
            //     .number()
            //     .typeError('Battery capacity must be a valid number')
            //     .when('$includeBattery', {
            //         is: true,
            //         then: (schema) => schema.required('Battery capacity is required').positive(),
            //         otherwise: (schema) => schema.notRequired(),
            //     }),

            // inverterCapacity: yup
            //     .number()
            //     .typeError('Inverter capacity must be a valid number')
            //     .when('$includeBattery', {
            //         is: true,
            //         then: (schema) => schema.required('Inverter capacity is required').positive(),
            //         otherwise: (schema) => schema.notRequired(),
            //     }),

            // includeBattery: yup.boolean().default(false),
        });
    }, [includeBattery]);
    ...
    const {
        ...
        defaultValues: {
            ...
            includeBattery: savedSolar.includeBattery || false,
        },
        resolver: yupResolver(schema, {
            context: { includeBattery },
        }),
    });
    ...
    return (
        <div className="grid grid-cols-12 gap-4">
            ...
            <div className="col-span-12 md:col-span-8 flex flex-col min-h-full mt-4">
                ...
                <AddToCardWedget totalPrice="200" onAddToCart={handleSubmit(onSubmit)} isLoading={isLoading} />
            </div>
        </div>
    );
};
