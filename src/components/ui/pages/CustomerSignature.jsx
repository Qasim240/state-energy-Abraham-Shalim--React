...
import { downloadIcon, infoCircleIcon, supportIcon } from '../../../../imagesPath';
...
const CustomerSignature = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('customer-signature');
    if (saved && sigCanvasRef.current) {
      sigCanvasRef.current.fromDataURL(saved);
  ...
  const handleEnd = () => {
    if (sigCanvasRef.current) {
      ...
      localStorage.setItem('customer-signature', dataUrl);
  ...
  const handleClear = () => {
    if (sigCanvasRef.current) {
      ...
      localStorage.removeItem('customer-signature');
      ...
      <div className='md:col-span-8  pr-2'>
          ...
          <nav className="flex py-3 text-gray-700" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
              <li className="inline-flex items-center">
                <a href="#" className="font-medium font-Avenir text-sm text-base-50">Cart Items</a>
              </li>
              <li className="inline-flex items-center">
                <a href="#" className="font-medium font-Avenir text-sm text-base-50">Customer Information</a>
              </li>
              <li className="inline-flex items-center">
                <a href="#" className="font-medium font-Avenir text-sm text-base-50">Loan Finance</a>
              </li>
        ...
        <div className='border border-secondary p-4 rounded-large lg:mt-4 '>
          ...
          <div className='text-end mt-4'>
            <PrimaryBtn
              className='bg-transparent px-6 py-[7px] border border-primary'
              onClick={handleClear}
            >
        ...
        <div className='mt-4'>
          <input
            ...
            type="checkbox"
          />
          <label className={`text-base ${fontMedium}`} htmlFor="terms">
            I acknowledge that I have reviewed and agree to comply with the terms & conditions outlined in the purchase order.
          </label>
        </div>
      </div>
